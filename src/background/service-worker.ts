...
import { connect, reconnect, getFullAccount, getAssetsByIds, getTransferFee, getAccount, getAccountHistory, getAccountNames, RQRX_NODES, type RawHistoryEntry } from "../lib/chain";
...
async function handleUnlock(
  ...
  let accountId = "";
  try {
    const acc = await getFullAccount(payload.accountName);
...
async function handleCreateAccount(
  ...
  let accountId = "";
  try {
    const acc = await getFullAccount(msg.accountName);
    accountId = acc?.account?.id ?? "";
  } catch {
    // Proceed.
  }
...
async function handleGetHistory(
  ...
  try {
    raw = await getAccountHistory(keys.accountId, limit);
  } catch (e) {
    return { error: formatChainError(e) };
  }
...
> {
  ...
  await ensureConnected();
  try {
    const acc = await getFullAccount(keys.accountName);
...
> {
  ...
  try {
    ...
    if (msg.toAccountName) {
      try {
        const acc = await getAccount(msg.toAccountName);
...
> {
  ...
  try {
    await ensureConnected();
    const acc = await getAccount(trimmed);
...
async function handleDappGetAccount(
  ...
  await ensureConnected();
  try {
    const acc = await getFullAccount(msg.name);
