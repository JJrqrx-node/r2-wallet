...
// We dig through the Apis instance to find the underlying browser WebSocket
// so we can probe whether the socket is actually usable. The shape varies
// slightly across SDK versions, so we try several known paths.
function getSocketState(): number {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const inst: any = (Apis as any).instance();
    if (!inst) return 3; // treat "no instance" as CLOSED
    const candidates = [
      inst?.ws_rpc?.ws,
      inst?.ws_rpc?.web_socket,
      inst?.ws_rpc?.socket,
      inst?.ws_rpc?.connection?.ws,
    ];
  ...
  // Unknown SDK shape: return -1 so callers can decide whether to trust the
  // cached promise. Treated as "healthy enough" so we don't reconnect needlessly.
  return -1;
}

export function isSocketOpen(): boolean {
  const s = getSocketState();
  // OPEN (1) is healthy; -1 means we couldn't probe, assume healthy.
  return s === 1 || s === -1;
}

export function connect(node: string = RQRX_NODES[0] ?? "wss://node01.rsquared.digital:8090"): Promise<void> {
  // If we have a cached connection for this node AND the socket is still
  // open, reuse it. Otherwise tear down and reconnect.
  if (connectPromise && activeNode === node && isSocketOpen()) {
    return connectPromise;
  }
  // Stale or dead — start fresh.
  if (connectPromise) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    try { (Apis as any).close(); } catch { /* ignore */ }
    connectPromise = null;
  }
  activeNode = node;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  connectPromise = p;
  // If the init promise rejects, clear cache so the next caller retries.
  p.catch(() => {
    if (connectPromise === p) {
      connectPromise = null;
      activeNode = null;
...
// Force a fresh reconnect to whatever the active (or fallback) node is.
// Service worker uses this when a chain call fails with a closed-socket error.
export async function forceReconnect(): Promise<void> {
  const node = activeNode ?? RQRX_NODES[0] ?? "wss://node01.rsquared.digital:8090";
